...
public:
    ...
    vector<double> GetACAAR(void){return ACAAR;}
    vector<double> CalACAAR();
    vector<double> CalCAAR();
    vector<double> AAR();
    Group(vector<string> tickerlist_,vector<double> ACAAR_,map<string,Stock> MyStock_):tickerlist(tickerlist_),num(tickerlist.size()),MyStock(MyStock_){}
    Group(){}
